...
7. An Agent Memory Layer prevents any fact from being computed twice across agents.
...
if .graph/ does not exist OR manifest.json is empty:
    mode = BOOTSTRAP
else if git diff {prev_commit} {curr_commit} returns changes:
    mode = INCREMENTAL
else:
    mode = COVERAGE_ONLY   ← no code change but coverage/mutation reports updated
```
...
At bootstrap time, JaCoCo and PIT reports do not exist. Coverage and mutation factors are forced to their worst-case values so every method is treated as a gap until proven otherwise:
...
This ensures the Risk Scorer ranks ALL methods as needing analysis. Once Wave 1 tests are generated and CI runs JaCoCo + PIT, the next run is INCREMENTAL and uses real coverage data.
...
`estimatedRunsToComplete` gives the team a realistic forecast of when full initial coverage will be achieved — important for project planning on a new ASIL-D codebase.
...
A shared in-memory knowledge bus that persists within a single orchestrator run. Agents write their findings to the memory layer; subsequent agents read it to avoid redundant analysis.
...
MutationAgent writes: confirmedEquivalentMutants, killTestIds
  → Coverage Agent reads: kill tests may cover additional branches (update coverage)
```
...
INIT
  ...
  ├── initialize AgentMemoryLayer(runId)
...
DISPATCH_LOOP
  ...
  │   ├── write output to AgentOutputCache
...
```
.graph/
├── bootstrap_state.json           ← NEW: wave tracking, resume state
├── mode.json                      ← NEW: BOOTSTRAP | INCREMENTAL | COMPLETE
├── ast/
├── cfg/
├── dfg/
├── call_graph/
